...
export function selectBestBowler(
  ...
  available.sort(
    (a, b) => (b.bowlingRating || 50) - (a.bowlingRating || 50)
  );
...
export function calculateBallOutcome(
  ...
  let lineMatch = false;
  if (delivery.line === 'OUTSIDE_OFF' && shot.direction === 'OFF')
    lineMatch = true;
  if (delivery.line === 'MIDDLE' && shot.direction === 'STRAIGHT')
    lineMatch = true;
...
export function applyBallResult(
  ...
  inn.overs =
    Math.floor(inn.legalBalls / 6) + (inn.legalBalls % 6) / 10;
  ...
  // End of over (legal balls only)
  const overJustEnded =
    !outcome.isExtra && inn.legalBalls > 0 && inn.legalBalls % 6 === 0;

  if (overJustEnded) {
    inn.oversHistory.push({
      bowlerId: bowlerIdThisBall,
      overs: Math.floor(inn.legalBalls / 6),
    } as InningsState['oversHistory'][number]);

    // Swap strike at end of over
    [inn.strikerId, inn.nonStrikerId] = [inn.nonStrikerId, inn.strikerId];

    const prevIds = inn.oversHistory.map(
      (o: { bowlerId: string }) => o.bowlerId
    );
  ...
  // Innings over: all out or overs exhausted
  const inningsOver =
    inn.wickets >= maxWickets || inn.legalBalls >= inn.maxOvers * 6;

  if (inningsOver) {
    inn.isCompleted = true;

    if (match.currentInnings === 1) {
      // Build 2nd innings; keep RESULT_SHOWCASE so client can show the last ball.
      // index.ts timeout will flip phase → AWAITING_DELIVERY after the showcase.
      ...
      // Do NOT set phase = AWAITING_DELIVERY here.
    } else {
      match.phase = 'MATCH_OVER';
